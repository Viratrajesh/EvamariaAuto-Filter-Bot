...
async def set_shortner(c, m):
    ...
    try:
        URL = m.command[1]
        API = m.command[2]
        resp = requests.get(f'https://{URL}/api?api={API}&url=https://telegram.me/Askmovies4').json()
...
async def donation(bot, message):
    ...
    yt=await message.reply_photo(photo='https://envs.sh/haY.jpg', caption=script.DONATE_TXT, reply_markup=InlineKeyboardMarkup(btn))
    await asyncio.sleep(300)
    await yt.delete()
