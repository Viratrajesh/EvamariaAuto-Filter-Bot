...
async def next_page(bot, query):
    ...
    if settings["link"]:
        ...
        btn = [[InlineKeyboardButton(text=f"🔗 {get_size(file.file_size)}≽ {get_name(file.file_name)}", url=f'https://telegram.dog/{temp.U_NAME}?start=file_{query.message.user.id}_{file.file_id}'),]
                for file in files
              ]
